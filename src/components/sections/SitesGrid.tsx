...
//  1. Sites Completos — Pizzaria, Clínica, Urban Store, Doçeria
//  2. Landing Pages   — Aurora Pet (slug começa com "landing/")

import Link from "next/link";
import { sites, formatPrice, type Site } from "@/lib/sites";

const ACCENTS_FULL:    string[] = ["#f97316", "#06b6d4", "#8b5cf6", "#ec4899"];
const ACCENTS_LANDING: string[] = ["#5E4AE3", "#FFB800", "#34C759", "#06b6d4"];

const fullSites    = sites.filter(s => !s.slug.startsWith("landing/"));
const landingSites = sites.filter(s =>  s.slug.startsWith("landing/"));

// ─── Card compartilhado ───────────────────────────────────────────────────────
function SiteCard({ site, accent }: { site: Site; accent: string }) {
  return (
    <article
    ...
    >
      ...
      <div
        ...
        style={{ height: "220px", background: `linear-gradient(145deg, ${accent}14, ${accent}05)` }}
      >
        {/* Grid */}
        <div
  ...
  return (
    <section id="sites" className="py-28 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* ══ SEÇÃO 1 — Sites Completos ══════════════════════════════════════ */}
        <div className="mb-16 text-center">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em]" style={{ color: "rgba(167,139,250,0.8)" }}>
            Portfólio
          </p>
          ...
          <p className="max-w-md mx-auto" style={{ color: "rgba(255,255,255,0.38)" }}>
            Sites multi-página com design exclusivo, código limpo e entrega
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {fullSites.map((site, i) => (
            <SiteCard key={site.id} site={site} accent={ACCENTS_FULL[i % ACCENTS_FULL.length]} />
          ))}
        </div>
        ...
        {/* ══ SEÇÃO 2 — Landing Pages ════════════════════════════════════════ */}

        {/* Badge explicativo */}
        <div
          className="mx-auto mb-10 max-w-lg rounded-2xl p-5"
          style={{
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-white mb-1">O que é uma Landing Page?</p>
              <p className="text-sm leading-relaxed" style={{ color: "rgba(255,255,255,0.45)" }}>
                É uma página só, com foco total em fazer o cliente entrar em contato.
                Sem menu cheio, sem distração. Só o essencial pra vender.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-16 text-center">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em]" style={{ color: "rgba(94,74,227,0.9)" }}>
            Novo
          </p>
          <h2 className="text-4xl sm:text-5xl font-black text-white mb-5 tracking-tight">
            Landing Pages
          </h2>
          <p className="max-w-lg mx-auto" style={{ color: "rgba(255,255,255,0.38)" }}>
            Uma página única focada em converter visitante em cliente.
            Ideal pra quem quer começar rápido, gastar menos e já sair vendendo.
            Sem complicação.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {landingSites.map((site, i) => (
            <SiteCard key={site.id} site={site} accent={ACCENTS_LANDING[i % ACCENTS_LANDING.length]} />
          ))}
        </div>
